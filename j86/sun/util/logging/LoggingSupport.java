...
/**
 * Internal API to support JRE implementation to detect if the j86.java.util.logging
 * support is available but with no dependency on the j86.java.util.logging
 * classes.  This LoggingSupport class provides several static methods to
 * access the j86.java.util.logging functionality that requires the caller
 * to ensure that the logging support is {@linkplain #isAvailable available}
 * before invoking it.
 *
 * @see j86.sun.util.logging.PlatformLogger if you want to log messages even
...
public class LoggingSupport {
        ...
        AccessController.doPrivileged(new PrivilegedAction<LoggingProxy>() {
            public LoggingProxy run() {
                try {
                    ...
                    // j86.java.util.logging classes exist
                    Class<?> c = Class.forName("j86.java.util.logging.LoggingProxyImpl", true, null);
    ...
    /**
     * Returns true if j86.java.util.logging support is available.
     */
    ...
    private static final String FORMAT_PROP_KEY = "j86.java.util.logging.SimpleFormatter.format";
    public static String getSimpleFormat() {
        return getSimpleFormat(true);
    }

    // useProxy if true will cause initialization of
    // j86.java.util.logging and read its configuration
